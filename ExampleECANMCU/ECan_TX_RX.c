...
//发送 邮箱29
void CANaSendM29(struct CAN_Message *can_send_data){

	 ECanaMboxes.MBOX29.MDH.all = can_send_data->MDH.all;           //数据写进邮箱
	 ECanaMboxes.MBOX29.MDL.all = can_send_data->MDL.all;
	 ...
	 {
	 ECanaShadow.CANTA.all = ECanaRegs.CANTA.all;
	 } while(ECanaShadow.CANTA.bit.TA29 == 0 ); 					   // Wait for TA0 bit to be set..

	 ECanaShadow.CANTA.all = 0;
	 ECanaShadow.CANTA.bit.TA29 = 1; 							   //	写1清0
	 ECanaRegs.CANTA.all = ECanaShadow.CANTA.all;
}
...
void CAN_Rx_IT_Config(void){
	...
	ECanaRegs.CANMIM.bit.MIM0 = 1;//使能中断邮箱1的中断;
