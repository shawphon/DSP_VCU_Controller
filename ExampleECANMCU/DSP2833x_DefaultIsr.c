...
extern void CANaSendM31(struct CAN_Message *can_send_data);
extern void CANaSendM30(struct CAN_Message *can_send_data);
extern void CANaSendM29(struct CAN_Message *can_send_data);
interrupt void INT13_ISR(void)     // INT13 or CPU-Timer1
{
	CpuTimer1.InterruptCount += 1;

	//	发送各节点消息	M1	邮箱31
	if(CpuTimer1.InterruptCount == 0){		//0 时发送
		CANaSendM31(&CANMessage_Tx);
	}

	//	发送各节点消息	M4	邮箱30
	if(CpuTimer0.InterruptCount == 1 || CpuTimer0.InterruptCount == 3 ||
			CpuTimer0.InterruptCount == 5 || CpuTimer0.InterruptCount == 7){
		if(CpuTimer1.InterruptCount == 2377){		//368+484+484+520+520 = 2376
			CANaSendM30(&CANMessage_Tx);
		}
	}

	//	发送各节点消息	M11	邮箱29
	if(CpuTimer0.InterruptCount == 2 || CpuTimer0.InterruptCount == 6){
		if(CpuTimer1.InterruptCount == 2897){	//‭368+484+484+520+520+520 =2896‬‬
			CANaSendM29(&CANMessage_Tx);
		}
	}
  ...
  // Remove after inserting ISR Code
	if(ECanaRegs.CANGIF1.bit.GMIF1 == 1){
		if(ECanaRegs.CANRMP.bit.RMP0 == 1){
			CpuTimer0.InterruptCount = ECanaMboxes.MBOX0.MDL.byte.CountBC;
			...
			ECanaRegs.CANRMP.bit.RMP0 == 1;			//发送成功应答寄存器 写1清0
		}
		ECanaRegs.CANGIF1.all = 0xFFFFFFFF;			//中断标志写1 清0
	}
