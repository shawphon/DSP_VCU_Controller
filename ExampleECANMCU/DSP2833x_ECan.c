...
void InitECana(void)		// Initialize eCAN-A module
{
    ...
    ECanaShadow.CANTIOC.bit.TXFUNC = 1;                         //  引脚功能使能，不然为一般I/O口
    ECanaRegs.CANTIOC.all = ECanaShadow.CANTIOC.all;

    ECanaShadow.CANRIOC.all = ECanaRegs.CANRIOC.all;
    ECanaShadow.CANRIOC.bit.RXFUNC = 1;       					 //  引脚功能使能，不然为一般I/O口
    ...
    #if (CPU_FRQ_150MHZ)
		/* The following block for all 150 MHz SYSCLKOUT (75 MHz CAN clock) - default. Bit rate = 500 kbps
...
void InitECanbGpio(void)
{
	EALLOW;
	// GpioCtrlRegs.GPAPUD.bit.GPIO30 = 0;	    // Enable pull-up for GPIO30 (CANRXA)
	GpioCtrlRegs.GPAPUD.bit.GPIO20 = 0;	    // Enable pull-up for GPIO16 (CANRXA)
	// GpioCtrlRegs.GPAPUD.bit.GPIO31 = 0;	    // Enable pull-up for GPIO31 (CANTXA)
	GpioCtrlRegs.GPAPUD.bit.GPIO21 = 0;	    // Enable pull-up for GPIO17 (CANTXA)

	/* Set qualification for selected CAN pins to asynch only */
	// Inputs are synchronized to SYSCLKOUT by default.
	// This will select asynch (no qualification) for the selected pins.
	// GpioCtrlRegs.GPAQSEL2.bit.GPIO30 = 3;   // Asynch qual for GPIO30 (CANRXA)
	GpioCtrlRegs.GPAQSEL2.bit.GPIO20 = 3;   // Asynch qual for GPIO18 (CANRXA)

	/* Configure eCAN-A pins using GPIO regs*/
	// This specifies which of the possible GPIO pins will be eCAN functional pins.
	// GpioCtrlRegs.GPAMUX2.bit.GPIO30 = 1;	// Configure GPIO30 for CANRXA operation
	GpioCtrlRegs.GPAMUX2.bit.GPIO20 = 3;	// Configure GPIO18 for CANRXA operation
	// GpioCtrlRegs.GPAMUX2.bit.GPIO31 = 1;	// Configure GPIO31 for CANTXA operation
	GpioCtrlRegs.GPAMUX2.bit.GPIO21 = 3;	// Configure GPIO19 for CANTXA operation
