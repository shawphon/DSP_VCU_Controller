...
extern void CANSendM31(struct CAN_Message *can_send_data);
extern void CANSendM30(struct CAN_Message *can_send_data);
extern void CANSendM29(struct CAN_Message *can_send_data);
interrupt void INT13_ISR(void)     // INT13 or CPU-Timer1
{
	...
	if(CpuTimer0.InterruptCount == 2 || CpuTimer0.InterruptCount == 4 ||
		...
		if(CpuTimer1.InterruptCount == 232){		//368+484+484+520=1856
			CANSendM31(&CANMessage_Tx);
		}
	}

	//	发送各节点消息	M12	邮箱30
	if(CpuTimer0.InterruptCount == 4 || CpuTimer0.InterruptCount == 8){
		if(CpuTimer1.InterruptCount == 297){		////‭368+484+484+520+520 = 2376
			CANSendM30(&CANMessage_Tx);
		}
	}
...
interrupt void ECAN1INTB_ISR(void)  // eCAN-B
{
  // Insert ISR Code here
	...
	{
		CpuTimer0.InterruptCount = ECanbMboxes.MBOX0.MDL.byte.CountBC;
		...
		ECanbRegs.CANRMP.bit.RMP0 = 1;			//发送成功应答寄存器 写1清0
	}
}
